const localeEn = {
  "experience": "Experience",
  "authentic_hospitality":
      "Experience the richness of Saudi hospitality as you immerse yourself in the culture and traditions of welcoming homes.",
  "discover": "Discover",
  "from_start_to_finish":
      'Discover local culture from start to finish with a native tour guide in their car!',
  "taste": "Taste",
  "popular_saudi": "The popular Saudi dishes and cuisine!",
  "enjoy_in": "Enjoy in",
  "saudi_arabia": "Saudi Arabia.",
  "start_tour": 'Start Exploring',
  "ajwad": "Ajwad",
  "noun": "noun [ plural ] 'ajwad",
  "societyGenerosity":
      "group of people who are known in Society for generosity and hospitality",
  "singler": "Singler",
  "familySaudi": "and they are Arethorisity or family Saudi host",
  "exploreSaudi":
      "Explore Saudi Arabia's rich cultural heritage with a Ajwady by your side.",
  "traditionalCrafts":
      "Whether you're interested in traditional crafts, food, music, dance, or history, there's something for everyone to discover.",
  "experienceSaudi": "Experience Saudi Arabia like a local.",
  "connectAjwady":
      "Connect with a Ajwady who will guide you through the country's cultural landmarks and provide you with a deeper understanding of its rich heritage.",
  "exploringSaudi": "Let's start exploring Saudi culture today!",
  "start": "Start",
  "selectType": "Welcome to Hido",
  "selectType3": "Unlock Saudi!",
  "selectType2": "Become an Experience Curator!",
  "selectTypeBreif":
      "Your gateway to authentic Saudi Arabia. Locals connect tourists",
  "selectTypeBreif2":
      "Turn your passion into income. Share your Saudi with tourists.",
  "selectTypeBreif3":
      "From breathtaking views to vibrant culture, discover it all and craft unforgettable memories!",
  "ajwady": "Tour Guide",
  "tourist": "Tourist",
  "local": "Local",
  "localGuide": "Local",
  "licensedTour":
      "licensed tour guide looking to offer your services to tourists ",
  "letsSignIn": "Lets Sign you in",
  "welcomeBack": "Welcome Back !",
  "phoneNum": "Phone Number",
  "incorrectPhoneNum": "Incorrect phone number. Please try again.",
  "rememberMe": "Remember Me",
  "forgotPassword": "Forgot Password?",
  "alreadyHaveAccount": "Already have an account?",
  "signIn": "Sign in",
  "login": "Login",
  'idIqama': 'ID',
  'birthDate': 'Date of birth',
  "or": "OR",
  "LoginGoogle": "Login with Google",
  "LoginTwitter": "Login with Twitter",
  "LoginApple": "Login with Apple ID",
  "haveAnAccount?": "Don’t have an account?",
  "letsSignUp": "Lets Register Account",
  "fullName": "Full name",
  "email": "Email",
  "yourEmail": "Enter your email",
  "password": "Password",
  "yourPassword": "Enter your password",
  "confirmPass": "Confirm password",
  "signUp": "Sign up",
  "lastStep": "Last Step",
  "nationality": "Nationality",
  "continue": "Continue",
  "verification": "Verification",
  "sendVerification": "We’ve sent you the verification code on ",
  "resendCodeIn": "Re-send again in",
  "resendCode": "Send again",
  "incorrectCode": "Incorrect code. Please try again.",
  "resetPassword": "Reset Password",
  "passwordReset":
      "Please enter your email address to request a password reset",
  'newPassword': "New Password",
  'typeNewPassword': "Enter new password",
  'licenseInformation': "License information",
  'nationalId': "National ID",
  'licenceExpDate': "License expiration date",
  'vehicleInformation': "Vehicle information",
  'youWillPickUpTourists':
      "You will pick up tourists in your own car. For security and safety purposes, we need you to provide us with your vehicle information",
  'vehicleNum': "Vehicle Serial Number",
  'nationalIdMustbe10': "National id must consist of 10 digits",
  "send": "Send",
  "add": "ADD",
  "search": "Search",
  "request": "Request",
  "accept": "Accept",
  "reject": "Reject",
  "letsRegisterAccount": "Lets Register Account",
  "map": "Map",
  "profile": "Profile",
  "whatYouWillAdd": "What is the thing you will add ?",
  "place": "Place ",
  "adventure": "Activity",
  "adventures": "Activity",
  "event": "Event",
  "tellUsAboutPlace": "Tell us about the place so we can add it ",
  "placeName": "Place name",
  "placeLoc": "Place Location",
  "aboutThePlace": "About the place ",
  "areYouExpertInPlace": "Are you Expert in the place ?",
  "price": "Price",
  "uploadImage": "Upload image",
  "addPlace": "Add Place",
  "tellUsAboutAdventure": "Tell us about the activity so we can add it ",
  "advName": "Activity name",
  "advLoc": "Activity Location",
  "aboutTheAdv": "About the Activity ",
  "today": "Today",
  "timeDate": "Time & Date",
  "tomorrow": "Tomorrow",
  "thisWeek": "This week",
  "chooseFromCalender": "Choose from calender",
  "addAdv": "Add Activity",
  "tellUsAboutEvent": "Tell us about the event so we can add it ",
  "eventName": "Event name",
  "eventLoc": "Event Location",
  "aboutTheEvent": "About the Event ",
  "addEvent": "Add Event",
  "done": "Done",
  "all": "All",
  "trip": "Tour",
  "festivals": "Festival",
  "events": "Local event",
  "restaurants": "Food",
  "hospitality": "Hospitality",
  "exclusiveDiscounts": "Exclusive Discounts for Holiday Inn Guests!",
  "happyExploring": "Happy exploring!",
  "where": "Where?",
  "seeAll": "See All",
  'saudiHospitality': 'Saudi hospitality',
  'craftTitle': 'Saudi Arabia\'s Finest Craft',
  'craftSubtitle':
      'From traditional handicrafts to modern fashion and electronics, our shop has it all.',
  'soap': 'Soap',
  'accessories': 'Accessories',
  'perfum': 'Perfum',
  'new': 'New',
  'chat': 'Chat',
  'tripDetails': 'Tour Details',
  'guests': 'Guests',
  'writeTripSchedule': 'What is the trip schedule',
  'adventureName': 'Activity Name',
  'time': 'Time',
  'total': 'Total',
  'totalPrice': 'Total price',
  'totalPayment': 'Total Payment',
  'perPerson': 'per person',
  'explore': 'Tours',
  'services': 'Experiences',
  'shop': 'Shop',
  'mohammedAli': "Mohammed Ali",
  'switchAccountToAjwadi': "Switch Account To Ajwadi",
  'switchAccountToTourist': "Switch Account To Tourist",
  'myProfile': "My account",
  "massage": "Messages",
  "myOrders": "My orders",
  "delivered": "Delivered",
  "onGoing": "On going",
  "bookmark": "Bookmark",
  "trips": "Tour",
  "paymentMethods": "Payment Methods",
  "contactUs": "Contact support",
  "terms": "Terms&Conditions",
  "signOut": "Sign out",
  "aboutMe": "About Me",
  "aboutMeBreif":
      "Enjoy your favorite dishe and a lovely your friends and family and have a great time. Food from local food trucks will be available for purchase. ",
  "interest": "Interest",
  "gamesOnline": "Games Online",
  "concert": "Concert",
  "music": "Music",
  "art": "Art",
  "movie": "Movie",
  "others": "Others",
  "update": "Update",
  "sameLocation": "Same Location",
  "change": "Change",
  "locBreif": "Street Number, Sector Number, City Name, State, India",
  "confirmLocation": "Confirm location",
  'tuwaik': "Tuwaik",
  "images": "Images",
  "invite": "Invite",
  "edgeOfTheWorldCliff": "Edge of the World Cliff",
  "riyadhSaudiArabia": "Riyadh,Saudi Arabia",
  "rating": "Rating",
  "distance": "Distance",
  "visit": "Visit",
  "aboutTheTrip": "About the tour",
  "aboutTheTripBrief":
      "Tuwig is mostly known for its fossilized coral reefs, shells, snails.ammonites, and other sedimentary organisms from 160 million years ago, found in the Al-Yamamah ",
  "readMore": "Read More ",
  "startFrom": "Start from",
  "buyTicket": "Buy Ticket",
  "shareWithFriends": "Share with friends",
  "copyLink": "Copy Link",
  "whatsApp": "WhatsApp",
  "facebook": "Facebook",
  "messenger": "Messenger",
  "twitter": "Twitter",
  "instagram": "Instagram",
  "skype": "Skype",
  "massages": "Chats",
  "cancel": "Cancel",
  "date": "Date",
  "confirm": "Confirm",
  "guests2": "Number of Guests",
  "peopleNo": "Number of People",
  "forMoreThan10": "For more than 7 you can choose the van as a ride",
  "checkForOneOur":
      "You can't book this experience because it starts in less than an hour",
  "pickUpLocation": "Pick up location",
  "pickUpRide": "Vehicle type",
  "sedan": "Sedan",
  "SUV": "SUV",
  "4x4": "4x4",
  "van": "Van",
  "ffx": "preliminary cost",
  "findAjwady": "Find Ajwady",
  "findLocal": "Find Locals",
  "myTickets": "Tickets",
  "myFav": "My Whish list",
  "notifications": "Notifications",
  "pastTrips": "Past",
  "upcomingTrips": "Upcoming",
  "canceled": "Canceled",
  "confirmed": "Confirmed",
  "waiting": "Waiting",
  "noTicket": "No Tickets Yet",
  "noTicketSub": "All your booking and request ticket will appear here",
  "noNotification": "No notifications Yet",
  "noNotificationsub":
      "It's empty now, but we'll let you know about any updates",
  "pay": "Pay",
  "discount": "Discount",
  "arabic": "Arabic",
  "english": "English",
  "howWasTheTrip": "How was the tour?",
  "writeHere": "Write here",
  "sorryForHearingThat": "Oh, sorry for hearing that",
  "tellUsWhatHappened": "can you tell us what happened",
  "searchforAjwady": "Search for Ajwady",
  "searchforLocal": "Search for Tour guide",
  "WeSentYourRequestAndWaiteTillAccepted":
      "We have sent your request and will let you know when it will be accepted.",
  "until": "Until",
  "offers": "Offers",
  "contactAjwadAppTeam": "Contact AjwadApp Team",
  "contactHidoTeam": "Contact Hido Team",
  "cancelTrip": "Cancel Tour",
  "trips2": "Tours",
  "review": "Rating",
  "acceptOffer": "Accept offer",
  "about": "About",
  "expert": "Expert",
  "reviews": "Reviews",
  "checkout": "Checkout",
  "creditAndDebit": "Credit / Debit Cards",
  "addNewCard": "Enter card information ",
  "other": "Other",
  "promocode": "Promocode",
  "cardNumber": "Card Number",
  "enter12digitCardNumber": "Enter 12 digit card number",
  "validThru": "Valid Thru",
  "cardHolderName": "Card Holder’s Name",
  "nameOnCard": "Name on Card",
  "addCard": "Add card ",
  "cvv": "CVV",
  "becomeAjwdy": "Become Ajwdy",
  "becomeTourist": "Become Tourist",
  "becomeAjwadyBreif":
      "Be a part of Ajwad and be come Ajwady and start rise our Saudi culture ",
  "switch": "switch",
  "youWantSiginOut": "Are you sure you want to sigin out ?",
  "serviceHospitalityTitle": "Salaam",
  "serviceHospitalitySubtitle":
      "Discover the Warmth and Generosity of Saudi Culture",
  "serviceEventTitle": "Saudia Happenings",
  "serviceEventSubtitle":
      "Stay Connected to the Latest Happenings in Saudi Arabia",
  "serviceFoodTitle": "Say”bamala”",
  "serviceFoodSubtitle":
      "Savor the Flavors of Saudi Arabia's Best Local Restaurants with Ease",
  "serviceAdventureTitle": "Saudis Activities",
  "serviceAdventureSubtitle":
      "Discover the Thrill of a Lifetime in Saudi Arabia",
  "cultureSights": "Culture Sights",
  'nearbyLocalRestaurants': 'Nearby Local Restaurants',
  'inviteYourFriend': 'Invite your friends',
  'getDiscount': 'Get 20% Discount',
  'nearestEvent': 'Nearest Event',
  'bag': 'My Bag',
  'emptyBag': 'Your bag is empty',
  'available': 'Available',
  'join': 'join',
  'startBook': 'book',
  'book': 'Book',
  'reserve': "Reserve",
  'date2': 'Date',
  'numberOfPeople': 'Number of people',
  'note': 'Note',
  'writeHere ': 'Write here ',
  'productInformation': 'Product information',
  'youMightAlsoLike': 'you might also like',
  'addToBag': 'Add to bag',
  'fieldRequired': '*Required Field',
  'invalidPhone': 'Invalid phone number',
  'invalidEmail': 'Invalid Email',
  'invalidPass': '*Password must be at least 8 characters',
  'ivalidPassAndConfirm': 'Password and confirm password are not matching',
  "No_Requests": "No Requests Yet",
  "From": "From",
  "To": "To",
  "addNewAdventure": "add New Activity",
  "addAdventure": "Add Activity",
  "addAdventureName": "Add Activity Name",
  "addAdventureFrom": "Add Activity From",
  "addAdventureTo": "Add Activity To",
  "addAdventurePrice": "Add Activity Price",
  "PENDING": "PENDING",
  "StartChat": "Start Chat!",
  'message': 'message',
  "EndRound": "Tour Ended",
  "EndTrip1": "Tour not ended",
  "cost": "Cost",
  "EndTrip": "Booking cancelled",
  "ContactTheHedoTeam": "Contact Hido Team",
  "km": "km",
  "sar": "SAR",
  "viewBooking": "View booking",
  "bookingValidation": "Make sure to select all fields",
  "somthingWentWrong": "Smothing went wrong , try again later",
  "commingSoon": "COMMEING \n SOON",
  "creditCard": 'Credit card',
  "placeBooked": 'Booked',
  "checkText":
      " It's a pleasure to accompany you, This is the itinerary that I suggested to you. Select what you want and create a memorable day! ",
  "showOffer": "View offer",
  "noMessages": "No New Message",
  "noMessagesSub": "All your messages with local guides appear here",
  "approximateLocation":
      "Approximate location , The exact location will be revealed after booking",
  "checkingForPayment": 'Checking for payment',
  "paymentFaild": 'payment faild , try again later ',
  "paymentSuccess": 'Your payment was Successful!',
  "totalNumOfGeusts": 'Total number of guests',
  "tripBooking": 'Tour booking',
  "youHaveReservationIn": 'You have reservation in',
  "male": 'Male',
  "female": 'Female',
  "preliminaryCost": 'Preliminary cost',
  "bookNow": 'Book now',
  "tourHistory": 'Tour history',
  "noTour": 'No tour yet',
  "deleteAccount": 'Delete ',
  "youWantDeleteYourAccount": 'Are you sure you want to delete your account ?',
  "exploreAsGuest": 'Explore as a guest',
  "whereWeWillBe": 'Location',
  "cancellationPolicy": 'Cancellation policy',
  "cancellationPolicyBreif":
      'Full refund: Cancellations made 48 hours or more before the tour departure time will receive a full refund.',
  "locationWillBeAvailableAfterBooking":
      'Location will be available after booking',
  "Evaluationoftheperson": 'What do you think about Mohammed As ajwady?',
  "mail": 'Send email',
  "tel": 'Call',
  "contact": "Contact Hido Team",
  "cansle": "Cancle Tour",
  "selectLocal": "Select local tour guide :",
  "tour": "Tours",
  "expertise": "Expertise",
  "noReview": "No Reviews yet",
  "noReviewSub": "All your reviews from our tourists will appear here",
  "HintMessage": "write your message here",
  "pricePerPerson": "price per person",
  "ItineraryDetails": "Itinerary Details",
  "BookingDetails": "Booking Details",
  "CancelBooking": "Cancel Booking",
  "CancelBookingConfirm": 'Are you sure you want to cancel your booking?',
  'Confirm': 'Confirm',
  "cancellationPolicyBreifAdventure":
      "Partial refund: Cancellations made between 24 and 48 hours before the booking time will receive a 50% refund.",
  "cancellationPolicyNote":
      "No refund: Cancellations made within 24 hours of the booking time will not be eligible for a refund.",
  "cancellationPolicySubNote":
      '  All refunds will be processed within 1-7 business days.',
  "notFind":
      "We couldn't find any local guides available for your chosen date and location",
  "noteDo": "Here is what you can do",
  "noteCansle": "You took a long time to accept an offer!",
  "Expand": "Expand Search",
  "notePrice": "Please note that the prices listed are per person",
  'mm/dd/yyy': 'DD/MM/YY',
  'reviewbooking': 'Review Booking',
  'hospitalitydetails': 'Hospitality Details',
  "numberofpeople": 'Number of People',
  "numberofpeorson": 'Number of People',
  'bookingdate': 'Booking Date',
  'applied': 'Applied',
  'addpromocode': 'Add Promocode',
  'ReviewRequest': "Review Request",
  'RequestedTourDetails': "Requested Tour Details",
  "saudiAdventure": "Saudi Activity",
  "serviceTitle": "Choose a unique experience",
  "hospitalityDetails":
      "Immerse yourself in Saudi customs and traditions of hospitality by sharing a meal with a local family.",
  "adventureDetails": "Immerse yourself in Saudi Activities with locals",
  "signAsTourist": "Sign in as Tourist ",
  'welcomeHido': "Welcom to Hido! ",
  "signUpTourist": "Sign Up As Tourist",
  "signUpLocal": "Sign Up As Local",
  "guestSubtitle": "Join us and get to have unique experience",
  "languages": "Spoken Language",
  "loclaInfo": "Local info",
  'edit': "Edit",
  'save': "Save",
  'intro1': 'Select the place you want to discover!',
  'intro2': 'Request tour & Receive Locals offers',
  'intro3': 'Choose offer & Confirm lane',
  'intro4': 'and  more to explore :)',
  'home': 'Home',
  'paymentMethod': 'Choose a payment method ',
  'pickUp': 'Pick up :',
  'dropOff': 'Drop off :',
  'online': 'Online',
  'offline': 'Offline',
  'itinerary': 'Itinerary',
  "atLeastItenrary":
      "*At least 3 activities are required to send the itinerary",
  "addActicity": "Add Activity ",
  "activityName": "Activity name",
  'startTime': 'Start time',
  'endTime': 'End time',
  'localProfile': 'Local Profile',
  "activityHint": "write the activity name",
  "activityError": "*this field is requested",
  "priceError": "*Please enter a price",
  "timeError": "*time is required",
  'delete': 'Delete',
  "reviewItenrary": "Review Itenrary",
  'offerSent': "Your Offer is Sent!",
  'next': 'Next',
  'offlineText':
      'You are currently offline and won‘t be receiving any tour requests',
  'noRequests': 'No Requests Yet',
  'noRequestsText': 'Our system is constantly checking for new requests.',
  'noExperience': 'No Experience',
  'noExperienceSub': 'All your registered experiences will appear here',
  'AddExperience': 'Add Experience',
  'kindofexperience': 'What kind of experience do you plan to offer?',
  'LocalEvent': 'Local Event',
  'hostSub':
      'Share your food and authentic Saudi customs with guests seeking a unique cultural experience',
  'adveSub':
      'As a local you will give the opportunity to the tourists to join you on your activities such as paragliding, or cycling, etc,.',
  'eventSub':
      'With your extensive knowledge, you will lead our guests on unforgettable activities through the treasures, history, and traditions of Saudi Arabia.',
  'Accepts': 'Accepts',
  'both': 'Both',
  'setPrice': 'Now, set your price',
  'changePrice': 'you can change it anytime',
  'Perperson': ' Per person',
  'Baseprice': 'Base price',
  'Hidofee': 'Hido fee',
  'Yourearn': 'Your earn',
  'UploadPhotos': 'Upload Photos',
  'uploadLimit': 'upload at least 3 photo',
  'Coverphoto': 'Cover photo',
  'Addmore': 'Add more',
  'Choosephotos': 'Choose photos',
  'hintLocation': "Riyadh,Al-Majma'ah, Saudi Arabia",
  'locationMSG':
      'Your address is only shared with the guests after confirming the reservation.',
  'locationCheck': 'Is the pin in the right spot ?',
  'Back': 'Back',
  'Next': 'Next',
  'Delete': 'Delete',
  'makeCover': 'Make it cover photo',
  'Review': 'Review',
  'Reviewexperience': 'Review the experience‘s details',
  'explination':
      'Before you submit, please review your details. Our team will translate your experience’s title and content into the other languages available.',
  'AvailableDates': 'Available Dates',
  'DD/MM/YYYY': 'DD/MM/YYYY',
  'ServedMeal': 'Served Meal',
  'Breakfast': 'Breakfast',
  'Lunch': 'Lunch',
  'Dinner': 'Dinner',
  'Publish': 'Send',
  'invalidNumber': '*The number is invalid ',
  'invalidDate': '*The date is invalid ',
  'invalidCvv': '*The cvv is invalid ',
  'cardholderName': 'Cardholder name',
  'hintHolder': 'Enter the holder name of this card',
  'expirationDate': 'Expiration Date',
  'dateHint': 'MM/YY',
  'person': "person",
  'persons': "Person",
  'adventuredetails': 'Activity Details',
  "GeneralInformation": "General Information",
  "Location": "Location",
  "PhotoGallery": "Photo Gallery",
  "GuestNumber": "Guest Number",
  "Date&Time": "Date & Time",
  "Price": "Price",
  "MyExperiences": "My Experiences",
  "noSummary": "No Summary yet",
  "noSummarySub":
      "The experience summary will be available 24 hours prior its date",
  'noBooking': "No Bookings yet",
  'noBookingSub': "All your accepted experience will show here",
  'noRequest': 'No Tours yet',
  'noRequestSub': 'All your accepted experience will show here',
  'people': 'Number of People',
  'seatHint': 'Number of seats',
  'locationCheckAdve': 'Is the pin in the right spot ?',
  'locationCheckEvent': 'Is the pin in the right spot ?',
  'EventDate': 'Available Dates',
  'Reviewevent': 'Review the experience‘s details',
  'explinationEvent':
      'Before you submit, please review your details. Our team will translate your experience’s title and content into the other languages available.',
  'Ontheway': 'On the way',
  'Arrived': 'Arrived',
  'Tourtime': 'Tour time',
  'Completed': 'Completed',
  "Tour": "Tour",
  "Hospitality": "Hospitality",
  "Adventure": "Activity",
  "Event": "Event",
  'Women': 'Women',
  'Men': 'Men',
  'Pepole': 'People',
  'hostType': 'Hospitality',
  'adventureType': 'Activity',
  'Reviewadventure': 'Review the experience‘s details',
  'explinationAdve':
      "Before you submit, please review your details. Our team will translate your experience’s title and content into the other languages available.",
  "ExperienceEdit": 'Experience Edit',
  'SaveChanges': 'Save Changes',
  'Summary': 'Summary',
  'signInLocal': 'Sign in as Local',
  'otp': 'OTP Code',
  'otpPhone': "We've sent the OTP code to number ",
  'phoneHint': 'Enter your phone number ',
  'tourSub':
      'With your extensive knowledge, you will lead our guests on unforgettable activities through the treasures, history, and traditions of Saudi Arabia.',
  'contactInfo': 'Contact Info',
  'welcometo': 'Welcome to',
  'hido': 'Hido',
  'idHint': 'Enter your National Identity ',
  'invalidId': 'invaild id',
  'providedServices': 'Provided Services',
  'welcome': 'Welcome',
  'serviceHint':
      'What services are you planning to provide? (at least choose 1 or more)',
  'exprinceHint':
      'Share your culture through hospitality, activity and local events .',
  'iban': 'IBAN',
  'ibanHint': 'Enter IBAN number',
  "drivinglicense": "Driving license",
  'drivingExpire': 'Driving license expiry date',
  'verfiy': 'Verify',
  'registerVehicle': 'Vehicle information',
  'vehicleLicense': 'Vehicle License',
  'vehicleLicenseHint': 'Enter the vehicle details',
  'vehicleHint': 'Enter the vehicle serial number',
  "invalidIBAN": '*invalid IBAN',
  'payment': "Payment",
  "signInTitle": 'Discover Saudi Arabia With Locals!',
  'editExperience': 'You can not edit this experience',
  'DateDuration': 'The date must be after the current date',
  'TimeDuration': "End time is less than start time",
  'StartTimeDurationError': 'Select a start time after current time',
  'StartTimeDuration':
      ' You must choose a start time that is later than the current time',
  "": "We've sent code to your number",
  "account": "Account",
  "legalDoc": "Legal document",
  "otpDriving": "We’ll send you an OTP to update your driving license",
  'tourLicense': "Tour Guide License",
  'guestLocalSignUp': 'Sign Up As Local',
  'createTourist': "Create account as Tourist ",
  'chooseNationality': 'Choose your nationality',
  "eventDetails": "Local Event Details",
  'accountNumberHint': "For notification , remainder , and help login",
  'delAccount': 'Delete account',
  'editPhoneNumber': 'Edit phone number',
  "phoneEditSubtitle": "We’ll text you code to verify your number ",
  'addProvidedServices': 'Add your provided services',
  'timerDialogContent':
      "no local guides offered tours matching your request. We suggest searching again later or exploring other dates.",
  'unfortunately': 'Unfortunately',
  'requestTimedOut': "Your tour request timed out!",
  'contentTimedOut': "Submit a new request to find your perfect activity.",
  'localOnway': 'Your local guide is on the way',
  "localOnwaySubtitle":
      'Be ready your local guide is on the way to the pick up location ',
  'localArrived': 'Your local guide arrived ',
  'localArrivedSubtitle': 'Please meet your guide at the pick up point ',
  'pickUpTitle': 'Rode with your local guide',
  'pickUpSubtitle': "It looks like you've met up with your tour guide. ",
  'tourStarted': 'Tour Started! ',
  'tourStartedSubtitle': 'We hope you have unforgettable time in your tour ',
  'tourCompleted': 'Your tour is completed !',
  'tourCompletedSubtitle':
      'Thank you for using our app! Please tell us about your experience ',
  'PickUp': 'Pick up',
  'tourTime': "Tour time ",
  'completeTour': 'Completed',
  'signOutDialog': "Active Tour Request",
  'signOutDialogContent':
      "There is ongoing tour request. To sign out, please complete or cancel your request",
  'submit': 'Submit',
  'howYourTour': "How was your tour in",
  'howYourExperience': "How was your experience in",
  'howYourGudie': "How was you guide",
  "noNextActivity": 'you don‘t have any activity yet',
  "noExpertise": "There are no expertise yet",
  "noAbout": "There are no informations yet",
  "NoReview": "There are no reviews yet",
  '*TheMinimumPrice': '*The minimum price for an activity is 30 SAR ',
  'timeErorrRange': "The time selected out of range",
  'eventDetailsSub': "Get to join the locals in their cultural events",
  "saudiEvent": 'Saudi Event',
  "uploadError":
      "Invalid image extension detected. Only 'jpg', 'jpeg', and 'png' are allowed",
  "imageError": 'Select at least three photos',
  "Region": 'Region',
  'tellUsMore': 'Tell us more about yourself ',
  'noExperiences': 'No Experiences Yet',
  'noExperiencesSubtitle':
      'Our local hosts are currently building exciting experiences. Check back soon to explore this region!',
  'accountUpadted': 'Account updated',
  "updateEmail": "Update email",
  'emailEditSubtitle':
      "We’ll send you an email to confirm your new email address",
  'emailConfirm': 'Confirm your email',
  'Arabic': 'Arabic',
  'English': 'English',
  'French': 'French',
  'German': 'German',
  'Chinese': 'Chinese',
  'Spanish': 'Spanish',
  'Russian': 'Russian',
  'EndTourTime': "The tour time hasn't ended yet",
  'StartTourTime':
      "The tour time hasn't started yet, you can start the tour 4 hours at max before its start time ",
  'pickUp1': 'Start time',
  'dropOff1': 'End time',
  'Alert': 'Alert!',
  'DeleteNote': 'You’re about to delete this experience',
  'DeleteDone': 'The experience has been deleted',
  'notDelete': 'The experience not deleted',
  'saveChange': "Changes have been saved successfully!",
  'imageValidSize': 'Image is too large, you can only upload less than 2 MB',
  're-enterPassword': 'Re-enter your password',
  'resetSucces': "Resetting password completed",
  'nextActivity': 'Your next activity ',
  'chat2': 'Chat',
  'seeMore': 'See more',
  'Yourservices': 'Experience Bookings',
  'summary': 'Summary',
  'Copied': 'Copied to clipboard',
  'Experiencetitle': 'Experience title',
  'regionChoose': 'Choose your Region',
  'pendingProgress': "Your tour is going to start soon ",
  'peopleNumber': 'People Number',
  'fullyBooked': "fully Booked",
  'ok': 'Ok',
  'termsOfService': 'Terms of Service',
  'termsText': "I agree to Hido's ",
  'EndTimeLess': "*Please select a date at least 48 hours from now",
  'noBookmarks': 'No Bookmarks',
  'emptyBookmarkText': 'All your bookmarks appear here',
  'PickMethod': 'You have to pick payment method',
  'ReplaceError': 'Replace the used number with another one',
  'ReplaceEmail': "replace the email you used with a different one",
  'noEndTrip': "Booking not canceled",
  'noPlace': 'place not found',
  'emailExists': 'Email already exists',
  'mobileExists': 'Mobile number already exists',
  'onTheWay': 'On the way',
  'passwordError': "you must enter at least 8 characters",
  'phoneOtpWrong': 'this otp for this user is wrong',
  'otpForbidden': "OTP Already Requested, Please Wait and try again.",
  'mobileNotFound': "mobile number is not found",
  'errorScreenTitle': 'Oops',
  'errorScreenSubTitle': 'Something went wrong',
  'errorScreenMainButton': 'Main page',
  'errorScreenReportButton': 'Report issue',
  'errorSheetTitle': 'Describe the issue you encountered',
  'errorSheetHint': 'Please provide a brief description of the problem',
  'offlineTitle': "No Internet connection",
  'offlineSubTitle': 'Please check your connection and try again',
  'errorFieldsAll': 'Please fill in all required fields',
  'vehiclePlate': 'Vehicle license plates',
  'vehicleType': 'Vehicle type',
  'invaildId': "Invaild id",
  'hidoVer': 'Hido Ver',
  'invalidCoupon': 'Invalid',
  'couponMiSpend': "This code is for amounts of",
  'orAbove': "or above",
  'Language': 'Language',
  'preferLang': 'Which language do you prefer',
  'wallet': 'Wallet',
  'vehicleInfo': 'Vehicle Info',
  'updateApp': 'Update Available',
  'updateAppContent':
      'You have to update to enjoy the new features and performance improvements',
  'showLess': "Show less",
  "apply": 'Apply',
  "guideTitle": "Where do you want to tour?",
  'guideSubtilte': "Explore the map and tap a pin to discover!",
  'experienceTitleAr': 'Experience title ( Arabic )',
  'experienceTitleEn': 'Experience title ',
  'descriptionAr': 'Description ( Arabic )',
  'descriptionEn': 'Description',
  'sortBy': 'Sort by',
  'byCity': 'By City',
  'newest': "Newest",
  'highestRated': 'Highest Rated',
  'priceFromLow': 'Price (Low to High)',
  'priceFromHigh': 'Price (High to Low)',
  'ContinueGuest': 'Continue as a Guest',
  'byGender': "By Gender",
  'shareSuccessful': 'Share was successful',
  'reset': 'Reast',
  'free': 'Free',
  'invalidSingIn': "This user cannot sign in as a tourist",
  'finished': 'Finished',
  'ExperienceSent': 'Experience Sent',
  'ExperienceSentDes':
      'Our team will review and we will work on publishing it as soon as possible',
  'ExperienceNotSent': "The experience wasn't published",
  'TryAgain': 'Try again',
  'UnderReview': 'Under review',
  'experiences': "Experiences",
  'pickService': "You must pick at least one services",
  'GuidanceLicense': "Tour Guide License",
  'UploadFile': "Upload File",
  'Transportation': "Transportation",
  'TransportationMethod': "Which transportation method do you usually use ",
  'CarDriveing': 'Car (I will be the one driving)',
  'CarNotDriveing': "Car (I won't be the one driving)",
  "pledge_responsibility":
      "I pledge to take full responsibility when transporting tourists using any means of transportation",
  "Skip": "Skip",
  'finish': 'Finish',
  'CompleteSaving': 'Complete saving information',
  'CompleteSavingDes':
      'Your entered information will be saved and can be modified later through the account page.',
  'Return': 'Return',
  'CompleteInfoDes':
      'To start accepting tour requests you have to complete you information',
  'CompleteInfo': 'Complete information',
  'fileValidSize': '*The file must be a PDF',
  'Addtion': 'Add',
  'imageSizeValid': 'Image too large. Maximum allowed size is less than 2GB.',
  'price include': 'Price includes',
  'addNewPoint': 'Add new item',
  'snack': 'Snack',
  'includeHint': 'Add brief description of the item',
  'priceIncludeDes':
      'In this section you add the items included in your pricing (This section is optional)',
};
